...
function appendBlogs(blogs){
  blogs.forEach(blog=>{
    blog_cont.innerHTML+=`<div class="blog my-16 px-10 w-ful flex flex-col justify-center">
        ...
        <button onclick="redirect('./blog.html?id=${blog._id}')" class="read-more flex items-center justify-center bg-[#ff1f78] mt-8 py-2 text-white rounded-md border border-[#ff1f78] hover:bg-transparent hover:text-black">Read more &#8594</button>
      </div>`;
  })
}
